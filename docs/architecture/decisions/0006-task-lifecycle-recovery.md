...
> 2026-09-11 更新：A/B 已分别由 PR #37/#38 合并。根据 [ADR-0007](0007-ci-artifact-cd-separation.md)，C 暂缓至 W11 独立 CI/CD 运行模型稳定，再同时适配两种运行。下文 B 的实现决策继续有效，C 是未交付契约。
...
2. **W07-B（已合并 PR #38）**：版本化重试策略、独立 attempt 表、数据库时间退避、手动重试和尝试历史界面。
