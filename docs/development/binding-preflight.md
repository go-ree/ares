...
B1 提供只读的 CI/CD 绑定意图预检，以及后续绑定/运行可复用的参数解析器。**不保存绑定，不创建运行或回执，不登记产物，也不调用旧工作流 fallback。** 仍为 epoch 9。B2 再实现绑定 schema/CAS/读写，B3 接独立运行上下文和回执；完整 B 尚未完成。
...
- 下一次 B2 增加专用绑定存储/CAS/最小权限及迁移验收、受控参数表单 DTO，不复用旧 app_config_workflows 表。B3 才接独立运行语义；C 产物能力就绪前不得接纳不可执行任务。
