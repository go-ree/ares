...
- 空库只读 status、epoch 1 schema bootstrap、顺序迁移至 epoch 7、重复 up 与 schema bootstrap 中断续跑；
...
发起数据库相关 PR 前还应在一次性隔离 Compose 环境记录完整 E2E 证据：`auth-secrets` 与新 volume 的完整依赖链及重复启动、旧 volume 在旧授权未撤销时零写入拒绝及 DBA 撤权后的升级、当前 epoch 7 的 22 张受管表、3 个 Demo 应用/4 个环境/12 个 AppConfig、runtime 对全部受管表的读取、只对其中 20 张表的精确 DML、`pipelines`/`pipelines_job_combination` 与 ledger 的写入拒绝、两张回执表只允许 `INSERT`、guarded migrator 成功/失败后均锁定且无会话，以及账号脚本对 mandatory roles、匿名/同名 Host、双密码、旧会话、出向 role/PROXY/DEFINER、管理员元数据权限/Restrictions、schema 可执行对象和外部入向外键的 fail-closed 行为。所有账号与迁移连接必须固定到同一 single-writer MySQL 8.4 实例；多写拓扑另需外部分布式互斥。备份恢复需证明当前 epoch 7 dump 可由支持 epoch 7 的 W06 候选二进制执行 `status`/`serve`，并分别保留 epoch 6、epoch 5、epoch 4 历史兼容夹具和每次迁移前 dump 的精确旧二进制恢复证据。旧二进制启动恢复仍是发布前本地 E2E，不由当前 GitHub Actions 自动执行。

W06 起，上述 Compose 证据的当前 schema 必须是 epoch 7；除原有 22 表与权限矩阵外，还要使用
`--scale ares=3` 启动三个 API/Worker 副本，完成 Noop 发布、任务数超过领取批量时的公平推进、
持有者退出后的租约接管、陈旧 fencing 写入拒绝、SIGTERM 有界 drain、integration revision 收敛和
v1 named leader 断连接管。基础 Compose 不发布后端宿主端口；单副本
`deploy/compose/api-debug.yaml` 不能用于三副本验收。只有保存容器数量、任务/租约数据库断言、
HTTP 结果和脱敏日志后，才能宣称多副本 Compose 验收通过。
