...
> - 文档类型：持续更新的开发路线与进度看板
> - 当前状态：**最高优先级为前端框架迁移**（[ADR-0008](../architecture/decisions/0008-frontend-react-semi-migration.md)）；B0 骨架与 B1 外壳已交付，W11-A3/B1/B2 已合并，B3 与 C 暂缓
> - 基线版本：`main@0e9eea2`，已合并 [PR #61](https://github.com/go-ree/ares/pull/61)
> - 最后更新：2026-09-16

## 2026-09-16：前端迁移调整为当前最高优先级

- 维护者决定：**前端框架迁移（ADR-0008 的 B0～B5 批次）为当前最高优先级，全速推进**；W11-B3 独立运行上下文与 W11-C 产物能力**暂缓**，待迁移完成后按原验收门禁恢复。
- 判定依据：ADR-0008 的迁移成本为月级（16.1k 行重写 + 6 个组件级 spec 重写），且迁移窗口内前端只接受 bugfix；W11-D 的新页面必须构建在新栈上，否则要重做一遍。迁移与后端 W11 链路不共享文件，B3/C 暂缓是排期让位而非范围放弃。
...
## 2026-09-16：前端迁移 B1 外壳（MainLayout / Login / Home）
...
- 验证：Vue 栈 21 文件 / 198 测试不退化，React 栈 4 文件 / 31 测试通过，`tsc`/`vue-tsc` 无错误，ESLint 与 Prettier 双栈通过，两条栈构建通过；测试期间实测修复了重复提交缺陷。
...
- 预览：部署代码 `05f06ce`（后续文档提交不重建业务镜像）；备份 `before-w11b2-05f06ce-epoch9.sql.gz`（gzip 校验通过）并保留 `ares-api:preview-epoch9`；备份在隔离 MySQL 8.4 恢复后关键行数一致、epoch 9 镜像兼容、epoch 10 镜像判定不兼容且 `serve` 拒绝启动。完整重建升级持久预览至 epoch 10，四个一次性任务退出 0，三个常驻服务 healthy，`migrate status` 兼容、dirty=0、29 张基础表。
...
- 接入类型目录创建/读取/启停 CAS、模板草稿创建/更新、不可变版本发布/读取以及三个有界 keyset 列表。ID/revision/版本号用十进制字符串，避免浏览器精度丢失；写回执只返回本次 revision，不追加不一致读取。
- 四角色可读元数据；完整草稿/规范和所有写入仅 admin，通过既有 workflows 权限、正式会话、Origin/CSRF、限流和审计。列表 SQL 不读取原始 JSON；完整读取记敏感审计，前置审计失败拒绝访问存储；错误/审计不回显 Spec 或 SQL。
- API 与资源/错误/审计边界见[管理契约](../development/template-catalog-api.md)，Swagger 同步。仍为 epoch 9，不修改冻结迁移/校验器，不新增页面，不启动任务或注册可信产物，不自动填充模板。
- 验证通过：后端全量测试/Vet、API 目录边界与真实 MySQL 联调 Race、完整 `make db-integration`、Swagger 生成、格式/工作流/Compose、62 处本地文档链接及 Docker API 构建。覆盖角色与泄漏、匿名/旧 token/CSRF、审计失败、身份防伪、分页/大小/JSON 严格解析、大整数、错误映射；MySQL 最小权限 HTTP 联调验证 CAS、固定版本、停用、分页及写后 schema 兼容。前端无改动，本轮未重跑前端验收；预览与 PR 链接交付时补充。
- 下一增量为 W11-B 应用 CI/环境 CD 固定版本绑定和独立运行；Java 真实步骤仍待 F。此后继续中文 PR 交付，不沿用上次文档 PR 的单次代合并授权。
- [中文 PR #59](https://github.com/go-ree/ares/pull/59) 已提交，未合并，云端检查以实时结果为准。预览部署 `53ec0b2`，三个服务 healthy、epoch 9 兼容；停机备份并校验 gzip，未进行新一轮恢复库演练。
- 实际登录冒烟：匿名目录 401，admin 登录后类型列表 200（2 条）、模板列表 200（空）；已注销本次测试会话。3 应用/2 用户/4 任务/2 类型/0 模板/0 版本保持不变。隔离测试容器及测试卷已清理，预览卷、账号和备份保留。

## 已合并：补齐持续预览文档（2026-09-14）

- 对比 PR #39 三个文件与当前 main：W07-B 已完成状态已包含；持续预览专门文档、索引入口和强制交付规则未包含，不能将 #39 直接视为已合并。
- 从最新主线重新整理缺失内容，补充当前 Origin 限制、已有账号保护、完整迁移与仅业务更新两条路径，以及纯文档/CI 改动无需重启的边界。不覆盖 W11 排期、不恢复 epoch 8 旧状态。
- 本轮仅文档；只读确认预览 `c40ace3` 的三个服务 healthy，不重启、不修改数据。新 PR 替代 #39；按维护者本次明确授权，在检查通过后合并新 PR 并关闭旧 PR，不扩展为未来自动合并授权。
- 替代评审：[PR #58](https://github.com/go-ree/ares/pull/58)；原始对比：[PR #39](https://github.com/go-ree/ares/pull/39)。54 处本地文档链接、代码围栏配对和 diff 空白检查通过；最终合并状态以 GitHub 为准。

## 已合并：CI 制品保留与依赖更新策略（2026-09-14）

- 原因：Dependabot 安全更新已关闭，但默认分支配置仍为六组生态安排每周版本更新；两个机制独立。
- 已关闭 Dependabot PR #45、#46、#47、#48、#49、#50、#52、#54、#55、#56；保留维护者自己的 PR。移除版本更新配置通过本轮中文 PR，合并后生效；不直接写 main。
- 仓库 Actions 制品/日志默认保留期已修改并回读确认为 7 天。上传步骤由 14 天改为 7 天，并补充独立每日清理工作流覆盖旧制品；不删除运行记录、缓存、Release 或镜像包。
- 已删除 55 个超过 7 天的旧 Actions 制品，共 13,176,601 字节；复查超期数量为 0，删除不可恢复。未删除运行记录或其他资源。
- `make workflow-check`（actionlint）、清理脚本 `bash -n` 与 `git diff --check` 通过；云端定时工作流待合并后生效，未声称已在 Actions 执行。仅 CI 配置与文档变动，无业务/schema/依赖版本变动，不重启本地预览，不关闭漏洞扫描门禁。[中文 PR #57](https://github.com/go-ree/ares/pull/57) 待维护者评审，未直接合并。

## 已合并：W11-A2b 事务存储与原子发布（2026-09-14）

- 新增独立 `templatecatalog` 内部服务：类型创建/读取/启停 CAS、模板创建/读取/草稿 CAS、固定版本读取及原子发布。稳定身份/归属不可变；同 revision 重复发布返回冲突。
- READ COMMITTED 与类型→模板固定锁序协调发布/停用；版本序号在父锁下分配，版本表不需 UPDATE 权限。数据库错误脱敏，失败整体回滚，校验 MySQL 排版后 JSON 大小。
- 无 schema 变更，仍为 epoch 9；不改历史迁移及冻结校验器。不接 HTTP/UI/执行器，不创建预览模板或触发任务。
- 新增 [Java CI 步骤计划](java-ci-step-catalog.md)，列出 Maven/Gradle、文件/OCI 发布与 CD 派生的参数和产物边界；真实执行仍待 F，下一增量为 A3 管理 API/权限/审计。
- 验证：后端全量测试/Vet、模板存储 Race（MySQL 连续三轮）、完整 `make db-integration`、Swagger/格式/工作流/Compose 检查通过；新增测试覆盖并发 CAS/发布、停用锁等待、失败回滚、不可变版本权限及存储后 JSON 限额，并在写入后重新检查 epoch 9 兼容性。Docker API 镜像构建通过。前端无改动，本轮未重跑前端验收。
- 预览已更新至 `c40ace3`：[localhost:8080](http://localhost:8080)，API/Web/MySQL 健康、epoch 9 兼容。停服务备份并校验 gzip；保留持久卷及原有 3 应用/2 用户/4 任务/2 类型，模板与版本仍为 0。本轮无迁移，不声称恢复库端到端演练。
- [中文 PR #53](https://github.com/go-ree/ares/pull/53) 待维护者评审，未直接合并；云端检查以 PR 实时结果为准。隔离测试 MySQL 容器及测试卷已清理，预览数据和备份保留。

## 已合并：本地管理员与密码规则（2026-09-14）

- 后续调整：新设密码禁止纯数字（包含 Unicode 十进制数字），仍至少 8 字符；不强制额外字符组合。admin 已通过正式改密接口更新并验证登录，旧会话按既有机制撤销；未修改 bearqy。密码不记录在文档或默认配置中。
- 后续验证：认证/API 测试、auth Vet、前端 21 文件/198 测试与类型/Lint/格式检查、Docker 构建通过。预览更新到 `096bf22` 后，新密码登录 200、纯数字改密被拒绝 400、旧密码登录 401，服务健康；更新同一 PR #51，未合并。

- 按维护者要求优先将新密码规则调整为至少 8 个 Unicode 字符，不允许纯数字；保留 1024 字节资源上限、Argon2id、登录限流与会话安全机制。首次设置及改密前后端同步；旧密码登录不追加新的最小长度校验。
- 本地持久预览新增 admin 管理员并验证登录成功，不修改已有 bearqy。账号只在本地数据库创建并记运维审计，不写入迁移、Demo、默认账号或提交密码/哈希。
- 本轮无 schema 变更，保持 epoch 9；W11-A2a 已合并，A2b 暂未推进。
- 验证：后端全量测试、auth/api 测试、auth Vet/Race、前端 21 文件/196 测试、类型/Lint/格式/构建及 Swagger/工作流/Compose 检查通过。
- 预览已部署 `ed60991`，先停服务备份并校验 gzip，保留数据和密钥；服务健康，重启后 admin 登录返回 200 且角色为 admin。账号创建只是本地维护操作，不成为项目默认凭据。
- [中文 PR #51](https://github.com/go-ree/ares/pull/51) 已由维护者合并；以上验证记录为该增量历史交付结果。

## 已合并增量：W11-A2a 类型与模板 schema

- 状态：已合并 PR #44。A2 拆为 A2a schema/迁移/权限与 A2b 事务存储/CAS；该增量不交付管理 API、UI 或新运行流程。
...
- 验证通过：后端全量测试/Vet、db 与模板 Race、Swagger/格式/工作流/Compose、完整 MySQL 8.4 迁移及账号动态集成。新增逐 DDL 恢复、存量种子保护、控制字符/未知字段/归属/版本摘要/序号校验和不可变版本权限测试已纳入 CI。前端代码无变动，本轮未重跑前端测试或声称新页面验收。
- 预览 `4f8a3f8` 已部署至 epoch 9，全部服务健康，原有 3 应用/1 用户/4 任务不变；Java/Python 各一条，模板/版本均为空，没有自动拆分旧流程。
- 升级前停止旧 Worker，数据库备份成功导入隔离空库；匹配 epoch 8 镜像检查兼容，关键行数一致。此为数据库恢复验证，不声称外部部署回退或恢复库业务端到端演练。备份与旧镜像保留；越过 epoch 9 后不得只降级镜像。
- [中文 PR #44](https://github.com/go-ree/ares/pull/44) 待验收；云端检查以 PR 实时结果为准，本轮不直接合并。
...
- 验证：后端全量测试与 Vet、相关 Race、前端 21 文件/195 测试、类型/ESLint/Prettier/生产构建、Swagger/格式/工作流/Compose 检查通过。
- MySQL 8.4：新增旧别名存量数据回归（正常编辑、真实名称匹配、别名不匹配、响应无旧字段、历史值未被修改）及完整 `make db-integration` 通过；已纳入 CI。
- 预览：代码 `796cf6b` 已部署；先停服务备份并校验 gzip 完整性（不等同恢复演练），保留数据及密钥卷。前后端/MySQL 健康，epoch 8 兼容，3 应用/1 用户/4 任务不变；实际 AppInfo 静态资源不含旧字段。
- PR：[中文 PR #43](https://github.com/go-ree/ares/pull/43) 已合并；上述记录是该次清理的交付范围和验证证据。
...
本文件同时承担路线图、进度看板和阶段验收清单三个职责。后续开发以工作包 `W01`～`W11` 为单位推进，每个工作包原则上对应一个可独立合并、可独立回退的中文 PR。
...
7. 每次业务开发交付后按[持续开发预览约定](../operations/development-preview.md)更新固定 Docker 预览，保留数据和密钥，记录实际部署 commit 与验证范围；交付结束不停止服务，预览不代表合并授权。纯文档/CI 配置变更记录仍在运行的版本，无需重启业务服务。
...
- W02 已用服务端会话、RBAC 与审计替换浏览器伪身份；本地全量门禁、真实部署验收、PR 自动化和主线合并均已完成。
...
新增 W11 为下一优先产品工作包，编号不代表 W10 后执行。顺序为 W11 文档评审与 A～E 模拟闭环 → W07-C / W08 → W11-F 真实接入 → W09/W10；详细门禁见[W11 实施计划](ci-artifact-cd-roadmap.md)。W07-A/B 的既有保证不回退。
...
| W07    | 重试、取消、超时与尝试历史   | W03、W06           | `开发中` | A：[PR #37](https://github.com/go-ree/ares/pull/37)；B：[PR #38](https://github.com/go-ree/ares/pull/38) | A/B 已完成；C 暂缓                  |
...
flowchart LR
    ...
    W07 --> W10
    W08 --> W10
    W06 --> W11[W11-A～E 模型与模拟闭环]
    W11 --> W07C[W07-C 适配独立运行取消]
    W11 --> W11F[W11-F 真实构建加工部署]
    W07C --> W11F
    W08 --> W11F
    W11F --> W09
    W11F --> W10
...
W02 与 W04 依赖 W01 已交付的仓库内质量基线，可以并行设计；W01 尚待维护者完成的许可证、私密报告渠道和仓库 Ruleset 管理项不阻塞后续代码工作。涉及同一数据库迁移或公共 API 时仍需按顺序合并。W09 不早于日志、可靠性和 Secret 契约稳定，避免为每个新平台重复返工。
...
- [x] 创建中文 PR、回填关联链接并通过自动化与维护者验收；PR #23 已合并。

2026-09-07 进度记录：[PR #23](https://github.com/go-ree/ares/pull/23) 已由维护者合并到 `main@2349e4a`，W02 完成定义全部满足，状态更新为 `已完成`。
...
- [x] 接受 [ADR-0003](../architecture/decisions/0003-generic-step-logs.md)，固定执行器分派、opaque cursor、SSE、安全资源和兼容边界。
- [x] 新增经过鉴权的 `GET /api/v1/tasks/:task_id/steps/:step_key/logs/stream`；唯一 query 字段为可选 `cursor`，同时兼容一致的 `Last-Event-ID`。
- [x] 服务端从任务步骤快照读取 `uses` 和 opaque `external_ref`，再通过 Registry 分派 `LogReader`。
- [x] `jenkins.job@v1` 实现 `LogReader` 并声明日志能力；客户端不能提交 Job、Build ID 或 Jenkins 地址。
- [x] Noop 等不支持日志的执行器返回明确能力结果，不伪造空日志。
- [x] 前端按任意数量步骤展示日志，只有 `capabilities.logs=true` 时提供入口。
- [x] 关闭详情或切换任务时立即释放流连接；断线从 cursor 续传。
- [x] 旧 Jenkins CI/CD 日志路由标记 deprecated，仅保留 v1 历史任务只读兼容。
- [x] 通用与旧日志路由共享连接配额、会话复验、最长时长、空闲期限、滚动写 deadline、脱敏审计和 context 取消边界。
...
- [x] 新增以 `config_id + ref + inputs` 为核心的发布 API。
- [x] 支持 `Idempotency-Key`，保存作用域、规范化请求摘要和创建结果。
- [x] 相同 key、相同请求返回原任务；相同 key、不同请求返回 409。
- [x] 旧 `app_name + env` 接口改为兼容适配层并复用同一领域服务。
- [x] 新增单个和批量发布预检，返回环境、AppConfig、工作流版本及步骤可用性。
- [x] 合并重复的发布 UI；选择环境后展示可发布应用及不可发布原因。
- [x] 批量发布逐项返回结果，进度基于真实步骤状态而不是固定 CI/CD 百分比。
...
- [x] 增加 `next_poll_at`、`lease_owner`、`lease_expires_at` 和单调 fencing token。
- [x] 领取、续租、保存结果、释放和终止操作全部校验 lease/fencing。
- [x] running 步骤的 Reconcile 也必须持有有效租约。
- [x] 实现公平到期扫描、退避与 jitter，移除通过更新 `updated_at` 轮转的临时策略。
- [x] Worker 生命周期接入进程 context，支持 graceful drain。
- [x] lease 续约失败时尽力取消当前本地执行 context，但不假设已经发出的外部请求一定可以被撤销。
- [x] 多实例的集成配置通过数据库 revision/CAS 自动收敛。
- [x] v1 遗留任务在排空前继续维持单副本或独立 leader 约束。
...
- 收到 SIGTERM 后立即停止领取，短任务在 drain 窗口完成；超时调用被取消且进程有界退出，租约到期后可由其他实例接管。
- 三个实例对集成设置的数据库 revision/CAS 自动收敛；慢旧探测不能覆盖新配置，落后实例在外呼前发现新 revision 并失败关闭或同步。
- v2 领取严格排除 v1；三个实例同时启动遗留排空循环时，只有数据库 leader 持有者可以访问旧 Jenkins，连接断开后可以接管。
...
| W07-C | 持久化取消、执行器确认协议、Jenkins 取消、Web 入口 | 未开始，暂缓至 W11-A～E 完成 |
...
| D-005 | 多副本交付语义     | `已确定` | [ADR-0005](../architecture/decisions/0005-multi-replica-worker-leases.md)：任务级 lease、数据库时钟和 fencing；执行器配合稳定幂等键 | W05、W06、W07 |
...
| R-008 | 前端缺少真实的自动化交互测试             | 中       | W02 已补身份、权限和用户管理关键路径测试及 Compose E2E         | 已关闭     |
...
PR #40/#41/#42/#43/#44/#51/#53/#57/#58/#59/#60/#61 已合并。W11-B2 固定版本绑定存储与管理 API 已交付并合并。维护者 2026-09-16 决定把**前端框架迁移列为当前最高优先级**（ADR-0008，批次 B0～B5），W11-B3 独立运行上下文与 C 产物能力暂缓，待迁移完成后恢复；D 阶段的新页面直接在新栈开发。W07-C 暂缓至新模型稳定；W01 管理项继续独立跟踪。构建部署能力仍未完成。
...
- 根据维护者要求，页面标题、导航标识、首页、前端包名/锁文件、启动日志与开发/生产健康响应统一为 Ares（机器标识使用 ares）。同步说明及遗留示例配置，移除旧品牌 API 域名，采用同源 API 路径。
- 新安装 Demo 前端仓库指向合并后的 go-ree/ares；不覆盖已有应用配置或历史数据。本次无 schema 迁移、无依赖版本升级。
- PR #40 已合并，W11 设计通过；本次为独立品牌调整，不启动 W11-A 业务开发。
- 验证：前端 194 项测试、类型检查、ESLint、Prettier、生产构建与后端 db/publish 测试通过；浏览器登录页标题为 Ares。已有 Element Plus 大分块提示保持不变。
- 预览：后端业务版本 `a0c5175`、前端版本 `995e36a`（追加图标资源改名），继续使用固定 `ares-preview`。已停旧服务并生成私有数据库备份，保留原数据库和密钥卷；更新前后均为 3 个应用、1 个用户、4 个任务，服务健康，checkup 返回 service=ares。
- 评审：[PR #41](https://github.com/go-ree/ares/pull/41)，未直接合并；云端检查以 PR 实时状态为准。后续品牌显示统一使用 Ares，机器名称使用 ares。

### 2026-09-11：应用类型 CI、产出物与 CD 解耦设计

- 用户确认：应用类型集中维护多套 CI 组合，CI 生成独立产出物，CD 可消费并加工产出物后部署；应用与动态环境仍保留。
- 新增 ADR-0007、领域设计及 W11-A～F 实施计划，明确模板版本、产物身份/发布门禁/父链、CD 最终部署引用、权限/幂等、旧模式 opt-in 与迁移验收。
- 校准 PR #38 已合并；W07-C 暂缓，W07-A/B 的 lease/attempt/超时安全边界继续复用。真实外部接入不绕过 W08，模拟演示不冒充真实构建部署。
- 本轮仅修改文档，未创建新 API/表、未变更 epoch 8、未运行新模型功能测试。现有 `ares-preview` 保留 `main@875cbd7` 和持久化数据；文档 PR 不重建同版本服务。
- [PR #40](https://github.com/go-ree/ares/pull/40) 已创建；8 份文档的 71 处本地链接和代码围栏校验通过，diff 空白检查通过。仅文档，不声称新业务测试通过；云端检查以 PR 实时状态为准。只有文档合并后才开始 W11-A 实现。持续预览文档 PR #39 仍独立评审，不在此 PR 代为合并。
...
- 分支与状态：`codex/w06-worker-leases` 基于 `main@1d7e5fc`；W06 的实现和本地完成定义已满足，中文 [PR #36](https://github.com/go-ree/ares/pull/36) 已创建且不会由开发任务直接合并，看板进入 `待验收`。
- 数据库与 Worker：epoch 7 为 v2 任务增加数据库调度时间、随机 owner 租约、单调 fencing token、饱和失败计数和公平扫描索引；Coordinator 的读取、Start/Reconcile 和全部结果写入均进入同一租约边界。续租丢失取消本地 context，SIGTERM 停止领取并进行有界 drain。
- 多副本配置：Jenkins/Kubernetes 设置使用 provider revision/CAS 和 context-aware 同步器收敛；Jenkins generation 变更还与任务 ClaimStep 共用 provider 事务 fence，活动任务不能被并发配置切换越过。落后副本在外呼前同步或失败关闭，慢旧探测不能覆盖新设置；disabled generation 仍可重新启用以恢复可证明归属的遗留任务。v1 遗留排空继续严格排除 v2，并在专用 MySQL 连接上使用零等待 named leader lock。
- 生命周期与兼容治理：HTTP listener 在任何后台领取前同步绑定，绑定失败不启动 Worker；Manager、Worker 与领取/取消竞态下的未启动租约释放共用首次停止时刻计算的绝对 drain deadline。v1 原始 SQL 对历史可空数值字段使用 nullable 扫描，并在任何 Jenkins 外呼前校验执行身份；畸形遗留行稳定失败关闭，不再阻塞整轮排空。
- Go、静态与供应链门禁：`gofmt`、`git diff --check`、`go mod tidy -diff`、`go mod verify`、`go test -count=1 . ./internal/...`、`go vet . ./internal/...`、`make race`、`make workflow-check`、`make compose-config` 与两次 `make swagger-check` 全部通过且未产生漂移。`make vuln` 未发现业务可达或已导入包漏洞；模块图中的 1 条上游报告不包含可达调用。
- 前端门禁：`make frontend-install`、`make frontend-check` 和 `make frontend-audit` 通过；Vitest 19 个测试文件共 188 项全部成功，ESLint、Prettier、TypeScript、生产构建和 npm audit 通过，audit 为 0 漏洞。构建仅保留现有 Element Plus 大 chunk 非阻塞提示。
- 真实数据库验收：MySQL 8.4.10 上 `make db-integration` 与 `make db-account-integration` 全部通过，覆盖 epoch 7 空库/epoch 6 升级、逐 DDL 中断恢复、并发与重复迁移、manifest/数据契约/最小权限账号；三个真实 Worker 并发唯一领取、饥饿边界、数据库时钟、过期接管、陈旧 token 全写拒绝、阻塞 Reconcile 重叠与幂等副作用一次，以及 Jenkins provider fence、revision/CAS、v1 named leader 断连接管和历史 NULL 安全收尾。
- Compose 验收：隔离空卷构建并启动 3 个 API/Worker 副本，Demo 基线为 4 个环境、3 个应用、12 份 AppConfig/流程/版本/绑定、4 个任务与 8 个步骤。完成首个 Noop 发布后精确停止 1 个副本，其余 2 个副本仍能读取原任务、创建并完成第二个发布；恢复为 3 个健康副本后，首个 key 的回执原样重放且带 `Idempotency-Replayed: true`。最终任务/步骤/回执/回执项为 `6/12/2/2`，两个新任务均 `succeeded`、fencing token 至少为 1 且租约字段清空；被停副本记录正常 Shutdown，日志未出现 bootstrap token 或测试密码。Compose 烟测证明可用性与跨副本幂等，持租任务接管/fencing 以此前真实 MySQL 可控阻塞用例为准，不混淆证据边界。
- 审查结论：并发、生命周期和完整代码终审发现的问题均已修复；最终复核未发现高置信 P1/P2。W06 仍未进入 `main`，R-003 在 [PR #36](https://github.com/go-ree/ares/pull/36) 合并前保持开放。

### 2026-09-08：W05 合并校准与 W06 设计启动

- 基线校准：[PR #35](https://github.com/go-ree/ares/pull/35) 已由维护者合并为 `main@1d7e5fc`；W05 更新为 `已完成`，R-005 随原子幂等发布进入主线关闭。
- 开发分支：`codex/w06-worker-leases` 从最新主线创建，不直接修改 `main`；W06 从 `未开始` 进入 `设计中`，关联 PR 待创建。
- 决策边界：新增 [ADR-0005](../architecture/decisions/0005-multi-replica-worker-leases.md)，固定 task 级数据库租约、数据库时钟、单调 fencing、公平到期扫描、有界退避、续租丢失取消和 graceful drain；Ares 明确只提供 at-least-once 调度，不承诺跨外部系统 exactly-once。
- 兼容范围：epoch 7 采用只增迁移；v2 写入全部进入租约边界，v1 遗留任务不误交给新 Worker，并以数据库级 leader 约束继续排空。多实例的集成设置将以数据库 revision/CAS 自动收敛，Redis/RabbitMQ 仍不是正确性依赖。
- 当时阶段：开始并行审计现有 Coordinator、MySQL migration/manifest、进程生命周期、集成设置热更新和故障测试入口；后续实现进展以上一条记录为准。
...
- 前端：删除重复批量页面，单发/批量共用 Release Composer；动态环境下按服务端资格选择 AppConfig，只提交本次预检通过项并冻结精确工作流版本。提交后对响应计数、顺序、目标、版本和结果不变量严格校验；网络、畸形响应、401/403/408/429、网关 5xx、处理中或结果未知均保留原 key/body。冻结内容绑定原 `actor_user_id`，重新认证后先刷新会话并核对主体，不允许换账号重放。提交中/结果不明确时阻止路由离开并提示页面卸载风险。
- 自动化门禁：全量 Go test/vet、关键包 Race Detector、模块校验、Swagger 可重复生成、govulncheck（0 个可达漏洞）、工作流与 Compose 配置检查、MySQL 8.4 完整 migration/最小账号矩阵均通过；前端 ESLint、Prettier、Type Check、生产构建、完整 Vitest 与 npm high/critical 审计均通过。
...
- 基线校准：[PR #34](https://github.com/go-ree/ares/pull/34) 已合并为 `main@aa37282`；W03 更新为 `已完成`，R-004 随通用步骤日志进入主线关闭。
- 开发分支：`codex/w05-idempotent-publish`，从最新主线创建，不直接修改 `main`；W05 从 `未开始` 进入 `设计中`，关联 PR 待创建。
- 当前审计：并行梳理单个/批量发布入口、AppConfig 与任务快照、数据库事务和 migration、前端重复界面、权限审计及幂等重放威胁；尚未修改生产代码。
- 设计顺序：先固化 `config_id + ref + inputs`、预检、`Idempotency-Key` 作用域与规范化请求摘要，再确定并发唯一约束、旧 `app_name + env` 适配、批量逐项语义、前端交互、升级与回退边界。
- 数据库影响：预计需要新增只增不删的 epoch 6 持久化幂等记录；表结构、保留期和兼容区间待 ADR 与现有 schema 审计完成后确认，不提前修改已发布的 epoch 1～5。
- 关联 PR：待创建；本记录只表示进入设计，不把 W05 范围标记为完成。
...
- 分支：`codex/w03-generic-step-logs`；基线为 [PR #23](https://github.com/go-ree/ares/pull/23) 合并后的 `main@2349e4a`。
- 状态校准：W02 已满足完成定义并更新为 `已完成`；R-001 与 R-008 随主线合并关闭。
- W03 当前阶段：[ADR-0003](../architecture/decisions/0003-generic-step-logs.md) 已接受，状态从 `设计中` 进入 `开发中`。契约固定为 `task_id + step_key + opaque cursor` 的鉴权 SSE；服务端只从步骤快照取得 `uses/external_ref` 并经 Registry 分派，Jenkins 地址绑定在外连前校验，v1 旧路由只读兼容，前端按任意步骤能力和单活动连接工作。
- 数据库影响：无。当前任务步骤快照已保存 `uses` 与 opaque `external_ref`；W03 不新增 epoch，不修改 epoch 1～5 manifest/checksum。
- 兼容与回退：canonical API 增量上线，旧 Jenkins 日志路由标记 deprecated 并拒绝 v2 任务；后端与前端需协调发布，回退时先回退前端再回退后端，无需恢复数据库。
- 实现顺序：通用日志服务与 Jenkins `LogReader` → 鉴权 SSE 与旧路由收紧 → 前端任意步骤日志 → 自动化、Compose 与完整门禁。
- 关联 PR：待创建；本记录不把设计中的事项标记为已交付。
