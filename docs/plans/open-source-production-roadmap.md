...
- `make workflow-check`（actionlint）、清理脚本 `bash -n` 与 `git diff --check` 通过；云端定时工作流待合并后生效，未声称已在 Actions 执行。仅 CI 配置与文档变动，无业务/schema/依赖版本变动，不重启本地预览，不关闭漏洞扫描门禁。中文 PR 待创建。
