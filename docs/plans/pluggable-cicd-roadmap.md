...
> 状态：PR #4 已交付阶段 A～C 与阶段 D 主链路；W03 [PR #34](https://github.com/go-ree/ares/pull/34) 已把通用步骤日志合并到主线，阶段 D 全部完成。W05 [PR #35](https://github.com/go-ree/ares/pull/35)、W06 [PR #36](https://github.com/go-ree/ares/pull/36) 已合并。当前推进 W07-A 超时与不明确结果处理，其余阶段 E 工作统一在 [开源化与生产能力开发计划](open-source-production-roadmap.md) 中跟踪。
...
- [x] 旧 CI/CD 字段只作为兼容投影。
- [x] 日志改为通过任务步骤读取，限制任意 Job 访问（W03 [PR #34](https://github.com/go-ree/ares/pull/34) 已合并）。
...
- [x] 为多副本 Worker 增加 `next_poll_at`、owner/lease、fencing、公平到期扫描、跨副本集成 revision 和 v1 leader（W06 已由 [PR #36](https://github.com/go-ree/ares/pull/36) 合并）。
...
2026-09-08 进度同步：W06 分支已完成 epoch 7、task lease/fencing、公平调度、续租丢失取消、
共享绝对 deadline 的 graceful drain、integration revision/CAS、Jenkins provider 事务 fence 和
...
可靠重试、取消和更多第三方执行器进入 [后续开发计划](open-source-production-roadmap.md)，不以不完整实现扩大首版风险；通用步骤日志已由 W03 [PR #34](https://github.com/go-ree/ares/pull/34) 合并进入主线。
...
- 三个 Worker 并发时任务租约唯一，过期接管递增 fencing token，陈旧持有者不能读取执行快照或写入结果。
- 三个副本自动收敛 integration revision；慢旧探测不能覆盖新设置，v1 leader 持锁连接断开后可以接管。
- 流程更新后运行快照不变。
...
- `docker compose up -d --build --wait --scale ares=3` 启动三个 API/Worker 副本；精确停止一个副本后，其余副本仍可读取旧任务、创建并完成新 Noop 发布，恢复后幂等重放零增量。
- 持租任务的过期接管、陈旧 fencing 拒绝、阻塞外部调用重叠与 SIGTERM 绝对 drain deadline 由真实 MySQL 和生命周期定向测试验证，不以快速 Noop Compose 烟测替代。
