...
func (pm *PublishManager) CreatePublish(req *CreatePublishRequest) (*entity.TaskRecord, error) {
	...
	// 更新指定id的数据
	a, err := db.Engine.ID(taskId).Update(&taskRecord)
	if err != nil {
...
func PublishingEntry(appName string, branch string, env string) error {
	var app []entity.Apps
	err := db.Engine.Where("app_name = ? AND deleted_at IS NULL", appName).Find(&app)
	if err != nil {
	...
	if len(app) > 1 {
		return fmt.Errorf("匹配到 %d 条记录信息，请检查app_name：%s 是否唯一存在", len(app), appName)
	}
	...
	err = db.Engine.Where("app_id = ? AND env = ? AND deleted_at IS NULL", app[0].AppId, env).Find(&appConfig)
	if err != nil {
		return fmt.Errorf("配置信息查询失败：%s", err)
	}
	if len(appConfig) == 0 {
		return fmt.Errorf("未找到对应环境配置信息，appId：%d,env：%s", app[0].AppId, env)
	}
	if len(appConfig) > 1 {
		return fmt.Errorf("匹配到 %d 条记录信息，请检查appId：%d, env：%s 是否唯一存在", len(appConfig), app[0].AppId, env)
	}
	//fmt.Printf("aaaa%+v\n", appConfig)

	// 检验对应环境的常量信息是否存在
	var envConfig []entity.EnvConfigs
	err = db.Engine.Where("env = ? AND deleted_at IS NULL", env).Find(&envConfig)
	if err != nil {
		return fmt.Errorf("配置信息查询失败：%s", err)
	}
	if len(envConfig) == 0 {
	...
	if len(envConfig) > 1 {
		return fmt.Errorf("匹配到 %d 条记录信息，请检查env：%s 是否唯一存在", len(envConfig), env)
	}

	// 这里需要构建实际的发布数据
	jenkinsParam, taskId, err := ComposePublishData(branch, app[0], appConfig[0], envConfig[0])
	if err != nil {
		return err
	}
	...
	// 确定该类型的项目应该走哪个发布管线
	var pipelines []entity.Pipelines
	err = db.Engine.Where("code_package_type = ? AND deleted_at IS NULL", appConfig[0].CodePackageType).Find(&pipelines)
	if err != nil {
	...
	if len(pipelines) > 1 {
		return fmt.Errorf("匹配到 %d 条记录信息，请检查code_package_type：%s 是否唯一存在", len(pipelines), appConfig[0].CodePackageType)
	}
	...
	taskRecord.CiBuildId = jobBuildId
	taskRecord.Status = "packaging"
	taskRecord.CiJobName = pipelines[0].JobName
	// 更新指定id的数据
	_, err = db.Engine.ID(taskId).Update(&taskRecord)
	if err != nil {
		return fmt.Errorf("ci阶段taskId回写失败：%s", err)
	}
